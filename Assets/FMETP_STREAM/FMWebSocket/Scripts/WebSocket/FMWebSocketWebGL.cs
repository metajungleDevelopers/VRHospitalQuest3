    ...
    public class FMWebSocketWebGL
    {
        ...
        [DllImport("__Internal")] private static extern void FMWebSocket_Close_2021_2_before();
            ...
            private void OnMessageCheck(string _msg)
            {
                ...
                else if (_msg.Contains("OnClientConnectedEvent("))
                {
                    ...
                    if (!existed)
                    {
                        ...
                        Manager.OnClientConnected(result);
                ...
                else if (_msg.Contains("OnClientDisconnectedEvent("))
                {
                    ...
                    for (int i = 0; i < ConnectedClients.Count; i++)
                    {
                        if (result == ConnectedClients[i].wsid)
                        {
                            ...
                            Manager.OnClientDisconnected(result);
                            Manager.OnConnectionCountChanged(ConnectedClients.Count);
                        }
                    }
                }
            ...
            void StartAll()
            {
                ...
                stop = false;
                StartCoroutine(ConnectAndAddEventListenersCOR(1f));
            }

            private IEnumerator ConnectAndAddEventListenersCOR(float _delaySeconds = 0)
            {
                if (_delaySeconds > 0) yield return new WaitForSecondsRealtime(_delaySeconds);
                yield return null; //have to wait for next frame after "OnEnable()" in first frame of Game Start, which cause connection bug
                ...
                StartCoroutine(ConnectionCheckerCOR(5f));
            }
            ...
            private bool GetWSConnectionStatus()
            {
                ...
                if (LastSeenTimeMS != 0)
                {
                    fmconnected = EnvironmentTickCountDelta(CurrentSeenTimeMS, LastSeenTimeMS) < 3000;
                }
                else { fmconnected = false; }

                return wsConnected;
            }
            IEnumerator ConnectionCheckerCOR(float _delaySeconds = 5f)
            {
                if (_delaySeconds > 0) yield return new WaitForSecondsRealtime(_delaySeconds);
                while (!stop)
                {
                    if (!GetWSConnectionStatus())
                    {
                        if (autoReconnect)
                        {
                            ...
                            {
                                FMWebSocket_Close();
                                StartCoroutine(ConnectAndAddEventListenersCOR(0f));
