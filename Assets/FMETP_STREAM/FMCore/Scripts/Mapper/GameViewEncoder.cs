...
namespace FMSolution.FMETP
{
    ...
    public class GameViewEncoder : MonoBehaviour
    {
        ...
        public int GetChunkSize { get { return OutputAsChunks ? chunkSize : maximumChunkSize; } }
        ...
        private IEnumerator ProcessCapturedTextureCOR()
        {
        ...
        private IEnumerator ProcessCapturedTextureGPUReadbackCOR()
        {
#if UNITY_2018_2_OR_NEWER
            if (rt != null)
            {
                //if(rt_gpu != null) Graphics.Blit(rt, rt_gpu);
        ...
        private IEnumerator DelayAddRenderPipelineListenersCOR(float delaySeconds = 0f)
        {
        ...
        private IEnumerator RenderTextureRefresh()
        {
            if (NeedUpdateTexture && !EncodingTexture)
            {
                NeedUpdateTexture = false;
        ...
        private IEnumerator EncodeBytes(byte[] RawTextureData = null)
        {
            ...
            if (OutputFormat == GameViewOutputFormat.RAWRGB24)
            {
                ...
                dataByteTemp = new byte[_length];
                Buffer.BlockCopy(RawTextureData, 0, dataByteTemp, 0, _length);
                EncodingTexture = false;

                GetChunkSize = 65000;
                int chunks = Mathf.CeilToInt((float)_length / (float)GetChunkSize);
                int _offset = 0;
                for (int i = 1; i <= chunks; i++)
                {
                    int dataByteLength = (i == chunks) ? (_length % GetChunkSize) : (GetChunkSize);
                    ...
                    OnRawRGB24ReadyEvent.Invoke(SendByte);
                    _offset += GetChunkSize;
                }
                */
            }
            else if (CapturedTexture != null || RawTextureData != null)
            {
                ...
                if (!detectedSimilarTexture)
                {
                    ...
                    int chunks = Mathf.CeilToInt((float)_length / (float)GetChunkSize);
                    int metaByteLength = CaptureMode == GameViewCaptureMode.Desktop ? 24 : 15;
                    for (int i = 1; i <= chunks; i++)
                    {
                        int dataByteLength = (i == chunks) ? (_length % GetChunkSize) : (GetChunkSize);
                        if (OutputFormat == GameViewOutputFormat.FMMJPEG)
