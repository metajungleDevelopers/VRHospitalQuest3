...
namespace FMSolution.FMETP
{
    ...
    public class GameViewEncoder_Editor : UnityEditor.Editor
    {
        ...
        SerializedProperty FMDesktopShowCursorProp;
        ...
        SerializedProperty FMDesktopTextureFormatProp;
        ...
        void OnEnable()
        {
            ...
            FMDesktopShowCursorProp = serializedObject.FindProperty("FMDesktopShowCursor");
            ...
            FMDesktopTextureFormatProp = serializedObject.FindProperty("FMDesktopTextureFormat");
        ...
        private void Action_SetSymbol()
        {
            ...
            bool _foundChanges = false;
            bool _foundSymbol = false;
            if (UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline != null)
            {
                //remove duplicated symbol in URP
                for (int i = 0; i < allDefines.Count; i++)
                {
                    if (allDefines[i].Contains("FMETP_URP"))
                    {
                        if (_foundSymbol)
                        {
                            allDefines.RemoveAt(i);
                            _foundChanges = true;
                            Debug.Log("Removed scripting define symbol: FMETP_URP");
                        }
                        _foundSymbol = true;
                    }
                }
            ...
            {
                //remove all related symbol when not using URP
                for (int i = 0; i < allDefines.Count; i++)
                {
                    if (allDefines[i].Contains("FMETP_URP"))
                    {
            ...
            if (UnityEngine.Rendering.GraphicsSettings.defaultRenderPipeline != null)
            {
                ...
                if (!_foundSymbol)
                {
                    ...
                    _foundChanges = true;
                    Debug.Log("Add scripting define symbol: FMETP_URP");
                }
            }

            if (_foundChanges) PlayerSettings.SetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup, string.Join(";", newDefines.ToArray()));
        }
                    ...
                    if (GVEncoder.CaptureMode == GameViewCaptureMode.Desktop)
                    {
                        ...
                        if (GVEncoder.FMDesktopShowAdvancedOptions)
                        {
                            GUILayout.BeginVertical("box");
                            {
                                ...
                                {
                                    GUILayout.BeginHorizontal();
                                    GUILayout.Label("= System Offset (" + GVEncoder.FMDesktopMonitorOffsetX + ", " + GVEncoder.FMDesktopMonitorOffsetY + ") Resolution (" + GVEncoder.FMDesktopFrameWidth + ", " + GVEncoder.FMDesktopFrameHeight + ")" + " Stride: " + GVEncoder.FMDesktopFrameStride);
                                    GUILayout.EndHorizontal();
                                }

                                GUILayout.BeginHorizontal();
                                EditorGUILayout.PropertyField(FMDesktopShowCursorProp, new GUIContent("Show Cursor"));
                                GUILayout.EndHorizontal();
                                GUILayout.BeginHorizontal();
                            ...
                            GUILayout.BeginVertical("box");
                            {
                                ...
                                GUILayout.BeginHorizontal();
                                EditorGUILayout.PropertyField(FMDesktopTextureFormatProp, new GUIContent("Texture Format"));
                                GUILayout.EndHorizontal();
                            }
                            GUILayout.EndVertical();
                        }
            ...
            GUILayout.BeginVertical("box");
            {
                ...
                else
                {
                    ...
                    GUILayout.BeginVertical("box");
                    {
                        ...
                        else if (GVEncoder.OutputFormat == GameViewOutputFormat.RAWRGB24)
                        {
